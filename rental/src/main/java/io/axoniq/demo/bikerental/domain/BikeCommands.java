...
public class BikeCommands {
    ...
    @CommandHandler
    public static void handle(RegisterBikeCommand command, EventAppender appender, ProcessingContext context) {
    ...
    @CommandHandler
    public String handle(RequestBikeCommand command, EventAppender appender, @InjectEntity Bike bike) {
        if (!bike.isAvailable()) {
